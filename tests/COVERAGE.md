# Nano Markup 1.0.0 conformance coverage
...
| --- | --- | --- | --- |
| Data model | Finite acyclic strings, mappings, and ordered sequences at any nesting position, including roots; duplicate sequence items remain significant; comparison is code-point exact | `root_raw_string`, `root_mapping`, `root_sequence`, `nested`, `sequence_duplicates`, `unicode_exactness`; native writer cycle tests | `duplicate_key`, `duplicate_nested_key`; native writer cycle rejection |
| Encoding | UTF-8 without a leading signature; interior U+FEFF data; representatives from every forbidden C0 range plus DEL and C1; boundary scalar values and noncharacters | Unicode in `root_mapping`, `unicode_exactness`, `interior_bom` | `utf8_bom`, `invalid_utf8`, `control_character`, `control_u000b`, `control_u000e`, `nul`, `del_control`, `c1_control` |
...
| Raw strings | Ordinary root and nested text; internal spaces, quotes, and `#`; mapping marker values after a separator; near-marker and suffix text; permitted Unicode whitespace is data rather than a blank line | `root_raw_string`, `root_mapping`, `root_unicode_whitespace`, `unicode_whitespace_nested`, `near_markers_sequence`, `raw_internal_spaces`, `mapping_scalar_markers`, `mapping_suffix_values`; writer reserved-root and mapping-string cases | `raw_leading_space`, `raw_trailing_space`, `root_raw_trailing_space`, `empty_after_separator`, `double_space_separator`, `double_space_block_header` |
...
| Multiline strings | Attached mapping `key|` headers; root and sequence `|` headers; nested empty blocks, leading/internal/trailing blanks, extra indentation, reserved text, Unicode whitespace content, space-only logical lines, and LF normalization | `root_multiline_string`, `root_empty_multiline_string`, `multiline_comments`, `multiline_indentation`, `multiline_edges`, `multiline_line_endings`, `unicode_whitespace_nested`; writer `root_space_only_lines`, `nested_space_only_lines` | `malformed_multiline_header`, `legacy_multiline_header_child`, `multiline_partial_indent`, `root_multiline_sibling` |
...
| Errors | Stable error categories and deterministic precedence across every adjacent priority pair | All valid fixtures | All invalid fixtures, especially `precedence_encoding_over_tab`, `error_precedence_tab`, and `precedence_*` |
