...
bare CR, a leading UTF-8 signature, invalid UTF-8, literal tabs, C0/DEL/C1
control bytes, or significant trailing whitespace. Git text normalization is disabled for those
...
`-text` in `.gitattributes` and selected Unicode-exactness fixtures.
`validate.py` requires every `-text` path to be protected and verifies every
recorded byte sequence before a release.
...
and [CONFORMANCE.md](../CONFORMANCE.md) for the decoder and writer adapter protocol.
